...
#define ESP_MEMPOOL_ARR_SIZE     OS_MEMPOOL_MEM_SIZE(ESP_BUFFER_BLOCK_COUNT, ESP_BUFFER_BLOCK_SIZE)

/* Memory Pool control block and memory space */
static uint8_t ESP_MemPoolCb[OS_MEMPOOL_CB_SIZE]    __ALIGNED(4) MEMPOOL_CC_ATTR;
static uint8_t ESP_MemPoolArr[ESP_MEMPOOL_ARR_SIZE] __ALIGNED(4) ;

/* Memory Pool for ESP parser */
const osMemoryPoolAttr_t ESP_MemPool_Attr = {
  .name      = "ESP Parser",
  .attr_bits = 0U,
  .cb_mem    = ESP_MemPoolCb,
  .cb_size   = sizeof(ESP_MemPoolCb),
  .mp_mem    = ESP_MemPoolArr,
  .mp_size   = sizeof(ESP_MemPoolArr),
};
