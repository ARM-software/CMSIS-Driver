...
*/
void ESP_Notify (uint32_t event, void *arg) {
  ...
  if (event == ESP_NOTIFY_EXECUTE) {
  ...
  else if (event == ESP_NOTIFY_CONNECTION_RX_DATA) {
    ...
    *u32 = rx_num;
  ...
  else if (event == ESP_NOTIFY_REQUEST_TO_SEND) {
    /* Received '>' character, ESP waits for data to transmit */
    osEventFlagsSet (pCtrl->evflags_id, WIFI_WAIT_TX_REQUEST);
  }
  else if (event == ESP_NOTIFY_CONNECTION_OPEN) {
    /* Retrieve connection properties (+LINK_CONN received) */
    ex = ESP_Resp_LinkConn (&stat, &conn);
    ...
    if (ex == 0) {
      ...
      if (conn.c_s == 1U) {
        /* ESP works as a server, put connection on the backlog */
        for (n = 0U; n < WIFI_SOCKET_NUM; n++) {
        ...
        /* ESP works as a client, find socket that initiated connection open */
        for (n = 0U; n < WIFI_SOCKET_NUM; n++) {
  ...
  else if (event == ESP_NOTIFY_CONNECTION_CLOSED) {
    /* Network connection is closed */
    ex = ESP_Resp_CtrlConn (&conn_id);
  ...
  else if (event == ESP_NOTIFY_STATION_CONNECTED) {
    /* Station connects to the ESP AP */
    ex = ESP_Resp_StaMac (mac);

    pCtrl->cb_event (ARM_WIFI_EVENT_AP_CONNECT, mac);
  }
  else if (event == ESP_NOTIFY_STATION_DISCONNECTED) {
    /* Station disconnects from the ESP AP */
    ex = ESP_Resp_StaMac (mac);

    pCtrl->cb_event (ARM_WIFI_EVENT_AP_DISCONNECT, mac);
  }
  else if (event == ESP_NOTIFY_CONNECTED) {
    /* ESP station connected to an AP */
    pCtrl->flags |= WIFI_FLAGS_STATION_CONNECTED;
  }
  else if (event == ESP_NOTIFY_GOT_IP) {
    /* ESP station got IP address */
    pCtrl->flags |= WIFI_FLAGS_STATION_GOT_IP;
  }
  else if (event == ESP_NOTIFY_DISCONNECTED) {
    ...
    switch (state) {
      ...
      case ARM_POWER_FULL:
        if ((pCtrl->flags & WIFI_FLAGS_INIT) == 0) {
          ...
          if (ex == 0) {
            /* Enable station */
            ex = ESP_Cmd_CurrentMode (AT_CMODE_SET, 1U);
          ...
          if (ex == 0) {
            ...
            ex = ESP_Cmd_ConnectionMux (AT_CMODE_SET, 1U);
  ...
  if ((module_info == NULL) || (max_len == 0U)) {
    ...
    if (ex == 0) {
      ...
      if (ex == 0) {
        ...
        if (ex == ESP_RESP_OK) {
          ex = ESP_Resp_GetVersion ((uint8_t *)module_info, max_len);
    ...
    switch (option) {
      ...
      case ARM_WIFI_MAC:                                // Station/AP Set/Get MAC;                                    data = &mac,      len =  6, uint8_t[6]
        if (len < 6) {
          ...
          if (interface == WIFI_INTERFACE_STATION) {
            /* Set station MAC */
            ex = ESP_Cmd_StationMAC (AT_CMODE_SET, (const uint8_t *)data);
          } else {
            /* Set AP MAC */
      ...
      case ARM_WIFI_IP_DNS2:                            // Station/AP Set/Get IPv4 secondary DNS address;             data = &ip,       len =  4, uint8_t[4]
        ...
        if (ex == 0) {
          ...
          ex = ESP_Cmd_DNS (AT_CMODE_SET, 1, ip_0, ip_1);
        }
        break;
      ...
      case ARM_WIFI_IP_DHCP:                            // Station/AP Set/Get IPv4 DHCP client/server enable/disable; data = &dhcp,     len =  4, uint32_t: 0 = disable, non-zero = enable (default)
        ...
        if (interface == WIFI_INTERFACE_STATION) {
          ex = ESP_Cmd_DHCP (AT_CMODE_SET, 1U, u32 != 0U);
        } else {
      ...
      case ARM_WIFI_MAC:                                // Station/AP Set/Get MAC;                                    data = &mac,      len =  6, uint8_t[6]
        if (*len < 6U) {
          ...
          *len = 6U;
          ...
          if (interface == WIFI_INTERFACE_STATION) {
            ex = ESP_Cmd_StationMAC (AT_CMODE_QUERY, NULL);
          } else {
      ...
      case ARM_WIFI_IP_DHCP:                            // Station/AP Set/Get IPv4 DHCP client/server enable/disable; data = &dhcp,     len =  4, uint32_t: 0 = disable, non-zero = enable (default)
        if (interface == WIFI_INTERFACE_STATION) {
          *len = 4U;
          ...
          ex = ESP_Cmd_DHCP (AT_CMODE_QUERY, 0U, 0U);
        } else {
          pu32 = (uint32_t *)data;
      ...
      if (ex == 0) {
        ...
        switch (option) {
          ...
          case ARM_WIFI_BSSID:                              // Station/AP Set/Get BSSID of AP to connect or of AP;        data = &bssid,    len =  6, uint8_t[6]
            ...
            ex = ESP_Resp_AccessPointMAC (pu8);
            break;
          ...
          case ARM_WIFI_IP_DHCP_LEASE_TIME:                 //         AP Set/Get IPv4 DHCP lease time;                   data = &time,     len =  4, uint32_t [seconds]
            ...
            if (interface == WIFI_INTERFACE_STATION) {
              ...
              ex = ESP_Resp_RangeDHCP (&u32, ip_0, ip_1);
        ...
        do {
          ...
          if (ex >= 0) {
            ...
            if (i < max_num) {
              ...
              switch (ap.ecn) {
                case ESP_ECN_OPEN:          ecn = ARM_WIFI_SECURITY_OPEN; break;
                case ESP_ECN_WEP:           ecn = ARM_WIFI_SECURITY_WEP;  break;
                case ESP_ECN_WPA_PSK:       ecn = ARM_WIFI_SECURITY_WPA;  break;
                case ESP_ECN_WPA2_PSK:
                case ESP_ECN_WPA_WPA2_PSK:  ecn = ARM_WIFI_SECURITY_WPA2; break;
                default:
                case ESP_ECN_WPA2_E:        ecn = ARM_WIFI_SECURITY_UNKNOWN; break;
              }
  ...
  if ((interface > 1U) || (config == NULL)) {
    ...
    if (interface == WIFI_INTERFACE_STATION) {
      ...
      do {
        switch(state) {
          case 0:
            ...
            ex = ESP_Cmd_StationIP (AT_CMODE_SET, ip_0, ip_1, ip_2);
            break;

          case 1:
            /* Connect to AP */
            ex = ESP_Cmd_ConnectAP (AT_CMODE_SET, config->ssid, config->pass, NULL);
            break;
        }
        ...
        if (ex != ESP_RESP_OK) {
          ...
          if (ex == 0) {
            ...
            if ((ex != ESP_RESP_OK) && (state == 1)) {
              /* Find out the reason for error */
              ex = ESP_Resp_ConnectAP (NULL);
      ...
      if (config->security == ARM_WIFI_SECURITY_WEP) {
        /* WEP encryption method is not supported by ESP8266 */
        rval = ARM_DRIVER_ERROR_UNSUPPORTED;
      }
      else {
        state = 0;
        ...
        do {
          switch (state) {
            case 0:
              ...
              ex = ESP_Cmd_CurrentMode(AT_CMODE_SET, mode);
              break;
            ...
            case 1:
              ...
              ap_cfg.max_conn  = 8; //Set ESP maximum supported
              ap_cfg.ssid_hide = 0; //(0:default)

              ex = ESP_Cmd_ConfigureAP (AT_CMODE_SET, &ap_cfg);
              break;

            case 2:
              /* Set access point MAC (must be different from station for ESP) */
              ex = ESP_Cmd_AccessPointMAC (AT_CMODE_SET, pCtrl->options.ap_mac);
              break;
            ...
            case 3:
              ...
              ex = ESP_Cmd_AccessPointIP (AT_CMODE_SET, ip_0, ip_1, ip_2);
              break;
            ...
            case 4:
              ...
              ex = ESP_Cmd_DHCP (AT_CMODE_SET, 0U, mode);
              break;
            ...
            case 5:
              if ((pCtrl->options.ap_dhcp_lease & (1U << 31)) == 0) {
                ...
                if (mode > 2880) {
                  /* Max lease time for ESP AP is 2880 */
                  mode = 2880;
                }
                mode |= (1U << 16);
              ...
              ex = ESP_Cmd_RangeDHCP (AT_CMODE_SET, mode, ip_0, ip_1);
              break;
          }
...
static int32_t ARM_WIFI_GetNetInfo (ARM_WIFI_NET_INFO_t *net_info) {
  ...
  if (net_info == NULL) {
    ...
    ex = ESP_Cmd_ConnectAP(AT_CMODE_QUERY, NULL, NULL, NULL);
  ...
  if ((socket < 0) || (socket >= WIFI_SOCKET_NUM)) {
    ...
    if (sock->type != ARM_SOCKET_SOCK_STREAM) {
      ...
      if (cnt < backlog) {
        ...
        ex = ESP_Cmd_TcpServerMaxConn (AT_CMODE_SET, (uint32_t)backlog);
        ...
        if (ex == 0) {
          /* Start TCP server */
          ex = ESP_Cmd_TcpServer (AT_CMODE_SET, 1U, sock->l_port);
    ...
    if (rval == 0) {
      ...
      if (sock->type == ARM_SOCKET_SOCK_DGRAM) {
        ...
        //if ((ip[0] == 0U) && (ip[1] == 0U) && (ip[2] == 0U) && (ip[3] == 0U)) {
          ...
          /* Open connection */
          ex = ESP_Cmd_ConnOpenTCP (AT_CMODE_SET, sock->conn_id, sock->r_ip, sock->r_port, 0U);
          ...
          if (ex != 0) {
            ...
            if (ex == 0) {
              ...
              else if (ex == ESP_RESP_ALREADY_CONNECTED) {
                rval = ARM_SOCKET_EALREADY;
              }
              else {
                if (ex == ESP_RESP_ERROR) {
    ...
    if (rval == 0) {
      if (sock->type == ARM_SOCKET_SOCK_DGRAM) {
        ...
        if (sock->state == SOCKET_STATE_BOUND) {
          ...
          ex = ESP_Cmd_ConnOpenUDP (AT_CMODE_SET, sock->conn_id, sock->r_ip,
                                                                 sock->r_port,
                                                                 sock->l_port,
                                                                 0U);
          if (ex != 0) {
    ...
    while (rval == 0) {
      ...
      if (sock->rx_len == 0) {
        ...
        if (sock->type == ARM_SOCKET_SOCK_DGRAM) {
          ...
          n += (uint32_t)BufRead (&pu8[n], cnt-n, &sock->mem);
...
static int32_t ARM_WIFI_SocketSendTo (int32_t socket, const void *buf, uint32_t len, const uint8_t *ip, uint32_t ip_len, uint16_t port) {
  ...
  if ((socket < 0) || (socket >= WIFI_SOCKET_NUM)) {
      ...
      if (sock->type == ARM_SOCKET_SOCK_STREAM) {
        ...
        if (rval == 0) {
          if (sock->state != SOCKET_STATE_CONNECTED) {
            ...
            /* Open connection */
            ex = ESP_Cmd_ConnOpenUDP (AT_CMODE_SET, sock->conn_id, r_ip,
                                                                   r_port,
                                                                   sock->l_port,
                                                                   0U);
            if (ex != 0) {
    ...
    if (rval == 0) {
      if (sock->type == ARM_SOCKET_SOCK_DGRAM) {
        /* 1. Message length is less than MAX (2048 bytes for ESP8266) */
        /* 2. For non-blocking socket, message can fit into buffer */
        /* 3. For blocking socket, message might need to be sent using multiple ESP_Send_Data calls */

        /* Initiate send operation */
        ex = ESP_Cmd_SendData (AT_CMODE_SET, sock->conn_id, len, r_ip, r_port);
        ...
        if (ex != 0) {
          ...
          while (num < len) {
            /* Determine amount of data to send (max 2048 bytes at once for ESP8266) */
            cnt = ESP_Send_GetFree();
          ...
          if (ex == 0) {
            ...
            if (ex == ESP_RESP_SEND_OK) {
              /* Packet sent, return number of bytes sent */
              rval = (int32_t)num;
            }
            else if (ex == ESP_RESP_SEND_FAIL) {
              /* Send failed */
              rval = ARM_SOCKET_ERROR;
            }
            else {
        ...
        while (rval == 0) {
          ...
          /* Initiate send operation */
          ex = ESP_Cmd_SendData (AT_CMODE_SET, sock->conn_id, cnt, r_ip, r_port);
...
static int32_t ARM_WIFI_SocketClose (int32_t socket) {
  ...
  if ((socket < 0) || (socket >= WIFI_SOCKET_NUM)) {
    ...
    if (sock->state == SOCKET_STATE_FREE) {
      ...
      ex = ESP_Cmd_TcpServer (AT_CMODE_SET, 0U, 0U);
    ...
    else if (sock->state == SOCKET_STATE_CLOSED) {
      ...
      if ((sock->state > SOCKET_STATE_LISTEN) && (sock->state < SOCKET_STATE_CLOSING)) {
        ...
        ex = ESP_Cmd_ConnectionClose (AT_CMODE_SET, sock->conn_id);
...
static int32_t ARM_WIFI_SocketGetHostByName (const char *name, int32_t af, uint8_t *ip, uint32_t *ip_len) {
  ...
  if ((name == NULL) || (ip == NULL) || (ip_len == NULL) || (*ip_len < 4U)) {
    ...
    ex = ESP_Cmd_DnsFunction (AT_CMODE_SET, name);
...
static int32_t ARM_WIFI_Ping (const uint8_t *ip, uint32_t ip_len) {
  ...
  if ((ip == NULL) || ((ip_len != 4U) && (ip_len != 16U))) {
    ...
    ex = ESP_Cmd_Ping (AT_CMODE_SET, ip, NULL);
  ...
  while (state < 4) {
    ...
    switch (state) {
      ...
      case 1:
        ...
        ex = ESP_Cmd_Echo (WIFI_ESP_ECHO);
        break;

      case 2:
        /* Reconfigure UART */
        ex = ESP_Cmd_ConfigUART (AT_CMODE_SET, br[k], 8, (1<<4)|(0<<2)|(0));
        break;
...
static int32_t GetCurrentDnsAddr (uint32_t interface, uint8_t dns0[], uint8_t dns1[]) {
  ...
  ex = ESP_Cmd_DNS (AT_CMODE_QUERY, 0, NULL, NULL);
