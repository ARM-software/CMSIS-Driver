...
#define ESP_AT_1_7_0_0    0x01070000
#define ESP_AT_1_6_2_0    0x01060200
#define ESP_AT_1_6_1_0    0x01060100
#define ESP_AT_1_6_0_0    0x01060000

#ifndef ESP_AT_VERSION
#define ESP_AT_VERSION  ESP_AT_1_6_2_0
#endif
...
/* Static functions */
static int32_t     ESP_ReceiveData (void);
...
static STRING_LIST_t List_PlusResp[] = {
  ...
  { "CWLIF"            },
  { "UART_CUR"         },
#if (ESP_AT_VERSION >= ESP_AT_1_6_0_0) && (ESP_AT_VERSION < ESP_AT_1_7_0_0)
  { "SYSMSG"           },
#elif (ESP_AT_VERSION >= ESP_AT_1_7_0_0)
  { "SYSMSG_CUR"       },
#endif
  { "CIPSTATUS"        },
  { "CIPDOMAIN"        },
...
/* Generic responses (see ESP_RESP_x definitions) */
static STRING_LIST_t List_ASCIIResp[] = {
  ...
  { "busy s..."         },
  { "ALREADY CONNECTED" },
/* Generic responses redirected to ESP_Notify function */
  { "WIFI CONNECTED"    },
  { "WIFI GOT IP"       },
  { "WIFI DISCONNECT"   },
/* Ignored */
...
/* GMR codes */
#define ESP_GMR_AT_VER            0
#define ESP_GMR_SDK_VER           1
#define ESP_GMR_COMP_TIME         2
#define ESP_GMR_BIN_VER           3
#define ESP_GMR_UNKNOWN           0xFF
...
/* Control codes */
#define ESP_CTRL_CONNECT        0
#define ESP_CTRL_CLOSED         1
#define ESP_CTRL_UNKNOWN        0xFF
...
void Serial_Cb (uint32_t cb_event) {
  ...
  if (cb_event & SERIAL_CB_TX_DATA_COMPLETED) {
    ...
    ESP_Notify (ESP_NOTIFY_TX_DONE, NULL);
  }

  if (cb_event & SERIAL_CB_RX_DATA_AVAILABLE) {
    /* Serial received data */
    ESP_Notify (ESP_NOTIFY_EXECUTE, NULL);
  }
}
...
/**
  Initialize ESP parser.
*/
int32_t ESP_Initialize (void) {
  ...
  mp_attr = ESP_MemPool_Attr;
  mp_id = osMemoryPoolNew (ESP_BUFFER_BLOCK_COUNT, ESP_BUFFER_BLOCK_SIZE, &mp_attr);
  ...
  if (err == 0) {
    /* Setup memory pool */
    BufInit (mp_id, NULL, &ESP.mem);
    BufInit (mp_id, NULL, &ESP.resp);

    /* Set initial state */
    ESP.state = ESP_STATE_ANALYZE;

    /*  Reset command counters, generic response value and command response code */
    ESP.cmd_cnt   = 0U;
    ESP.cmd_sent  = CMD_UNKNOWN;
    ESP.gen_resp  = 0U;
    ESP.resp_code = CMD_UNKNOWN;
    ESP.resp_len  = 0U;
  }
...
/**
  Uninitialize ESP parser.
*/
int32_t ESP_Uninitialize (void) {
...
/**
  Set serial baudrate.
*/
int32_t ESP_SetBaudrate (uint32_t baudrate) {
...
void ESP_Execute (void) {
  ...
  while (sleep == 0) {
    ...
    /* Receive serial data */
    if (ESP_ReceiveData() != 0U) {
      /* Out of memory */
      ESP_Notify (ESP_NOTIFY_OUT_OF_MEMORY, NULL);
    }

    switch (ESP.state) {
      case ESP_STATE_ANALYZE:
        ESP.state = AnalyzeLineData();
        break;
      ...
      case ESP_STATE_RECV_DATA:
        /* Copy IPD data */
        /* Set pointer to source memory buffer */
        p = (uint32_t)&ESP.mem;

        /* Call notify using pointer to ESP memory buffer */
        ESP_Notify (ESP_NOTIFY_CONNECTION_RX_DATA, &p);
      ...
      case ESP_STATE_RESP_DATA:
        /* Received +CMD response */
        if (ESP.resp_code == CMD_IPD) {
          /* Copy response (including ':' character) */
          BufCopy (&ESP.resp, &ESP.mem, ESP.resp_len+1);

          /* Receive network data (+IPD) */
          ESP.ipd_rx = 0U;

          ESP_Notify (ESP_NOTIFY_CONNECTION_RX_INIT, &ESP.ipd_rx);

          if (ESP.ipd_rx != 0) {
            /* Start receiving data */
            ESP.state = ESP_STATE_RECV_DATA;
          }
          else {
            /* Flush +IPD response */
            ESP.state = ESP_STATE_FLUSH;
          }
        }
        else {
          /* Response data arrived */
          BufCopy   (&ESP.resp, &ESP.mem, ESP.resp_len+2);

          ESP.state = ESP_STATE_ANALYZE;

          if (ESP.resp_code == CMD_LINK_CONN) {
            /* Connection established (+LINK_CONN) */
            ESP_Notify (ESP_NOTIFY_CONNECTION_OPEN, NULL);
          }
          else if (ESP.resp_code == CMD_STA_CONNECTED) {
            /* Station connected to ESP AP (+STA_CONNECTED:<sta_mac>) */
            ESP_Notify (ESP_NOTIFY_STATION_CONNECTED, NULL);
          }
          else if (ESP.resp_code == CMD_STA_DISCONNECTED) {
            /* Station disconnected from ESP AP (+STA_DISCONNECTED:<sta_mac>) */
            ESP_Notify (ESP_NOTIFY_STATION_DISCONNECTED, NULL);
          }
          else if (ESP.resp_code != CMD_UNKNOWN) {
      ...
      case ESP_STATE_RESP_GEN:
        /* Generic response received */
        if (ESP.cmd_cnt > 0) {
          ESP.cmd_cnt -= 1U;
        }

        if (ESP.gen_resp == ESP_RESP_UNKNOWN) {
          /* Unknown response */
          //
        }
        else if (ESP.gen_resp == ESP_RESP_ECHO) {
          /* Echo response */
          //
        }
        else if ((ESP.gen_resp == ESP_RESP_BUSY_P) || (ESP.gen_resp == ESP_RESP_BUSY_S)) {
          /* Busy processing or busy sending */
          ESP.state = ESP_STATE_FLUSH;
        }
        else if (ESP.gen_resp == ESP_RESP_WIFI_CONNECTED) {
          ESP_Notify (ESP_NOTIFY_CONNECTED, NULL);
        }
        else if (ESP.gen_resp == ESP_RESP_WIFI_GOT_IP) {
          ESP_Notify (ESP_NOTIFY_GOT_IP, NULL);
        }
        else if (ESP.gen_resp == ESP_RESP_WIFI_DISCONNECT) {
          ESP_Notify (ESP_NOTIFY_DISCONNECTED, NULL);
        }
        else {
          if ((ESP.cmd_sent != CMD_UNKNOWN) && (ESP.cmd_cnt == 0)) {
            /* Application waits for response */
            ESP_Notify (ESP_NOTIFY_RESPONSE_GENERIC, NULL);
          }
        }
      ...
      case ESP_STATE_SEND_DATA:
        ...
        if ((ESP.cmd_sent == CMD_CIPSEND)  && (ESP.cmd_cnt == 0)) {
          /* Last command was send data, application wants to transmit */
          ESP.gen_resp = ESP_RESP_TXREQ;

          /* Received '>' character */
          ESP_Notify (ESP_NOTIFY_REQUEST_TO_SEND, NULL);
      ...
      case ESP_STATE_RESP_CTRL:
        /* Control code arrived */
        if (ESP.ctrl_code == ESP_CTRL_CONNECT) {
          /* <conn_id>,CONNECT */
          ESP_Notify (ESP_NOTIFY_CONNECTION_OPEN, NULL);
        }
        else if (ESP.ctrl_code == ESP_CTRL_CLOSED) {
          /* <conn_id>,CLOSED */
          ESP_Notify (ESP_NOTIFY_CONNECTION_CLOSED, NULL);
        }
...
#define ESP_LINE_COLON        (1U << 3) /* Line starts with ':' character       */
#define ESP_LINE_ASCII        (1U << 4) /* Line starts with ASCII characters    */
#define ESP_LINE_CRLF         (1U << 5) /* Line starts with CRLF characters     */
...
/**
  ...
  \return next parser state, see ESP_STATE_ definitions.
*/
static uint8_t AnalyzeLineData (void) {
  uint8_t  crlf[] = {'\r', '\n'};
  uint8_t  code;
  ...
  if ((flags & ESP_LINE_NODATA) || (flags & ESP_LINE_INCOMPLETE)) {
    ...
    if (flags & ESP_LINE_COLON) {
      ...
      ESP.resp_code = GetCommandCode (pMem);

      if (ESP.resp_code == CMD_IPD) {
        /* Receive network data (+IPD) */
        /* Find colon, there is no CRLF after +IPD */
        ESP.resp_len = (uint8_t)BufFindByte (':', pMem);

        rval = ESP_STATE_RESP_DATA;
      }
      else {
      ...
      if (n == -1) {
        ...
        /* This is response to ping command */
        ESP.resp_code = CMD_PING;
    ...
    if (flags & ESP_LINE_CRLF) {
      ...
      if (code != ESP_GMR_UNKNOWN) {
        rval = ESP_STATE_RESP_GMR;
      }
      else {
        code = GetASCIIResponseCode (pMem);

        if (code != ESP_RESP_UNKNOWN) {
          /* Generic response received */
          rval = ESP_STATE_RESP_GEN;
        }
        else {
          /* Unknown */
          rval = ESP_STATE_FLUSH;
...
static uint8_t GetCommandCode (BUF_LIST *mem) {
  ...
  for (i = 0; i < maxi; i++) {
    ...
    if (val > 0) {
      /* String matches */
      if (i == CMD_IPD) {
        /* +IPD does not have CRLF terminator */
        /* Return command code                */
        code = i;
      }
      else {
        /* Check if line terminated */
        if (BufFind (crlf, 2, mem) != -1) {
          /* Line terminator found */
          /* Return command code   */
          code = i;
        }
...
/**
  ...
  \return Generic response code, see ESP_RESP_ definitions
*/
static uint8_t GetASCIIResponseCode (BUF_LIST *mem) {
  uint8_t i, maxi, code;
  int32_t val;

  code = ESP_RESP_UNKNOWN;
...
static int32_t GetRespArg (uint8_t *buf, uint32_t sz) {
  ...
  if (BufPeekByte(&ESP.resp) == '+') {
    /* Sync till the first ':' after +command string */
    do {
      val = BufReadByte (&ESP.resp);
  ...
  if (val == 2) {
    ...
    /* Clear '\n' character */
    BufFlushByte (&ESP.resp);

    /* Peek what is next */
    b = (uint8_t)BufPeekByte (&ESP.resp);
...
*/
int32_t ESP_Resp_IPD (uint32_t *link_id, uint32_t *len, uint8_t *remote_ip, uint16_t *remote_port) {
  char    *p;
  uint8_t  buf[32];
  int32_t  val;
...
*/
int32_t ESP_Resp_LinkConn (uint32_t *status, ESP_LINK_CONN *conn) {
  char    *p;
  uint8_t  buf[32];
  int32_t  val;
  uint32_t a;     /* Argument counter */
...
*/
int32_t ESP_Resp_CtrlConn (uint32_t *conn_id) {
  int32_t val;
  uint8_t b;

  val = BufReadByte (&ESP.mem);
...
*/
int32_t ESP_Resp_StaMac (uint8_t mac[]) {
  char    *p;
  uint8_t  buf[32];
  int32_t  val;
...
/**
  ...
  \return generic response code ESP_RESP_x
*/
int32_t ESP_Resp_Generic (void) {
...
/**
  ...
  \return 0:OK, -1: error
*/
int32_t ESP_Cmd_GetVersion (void) {
  char out[16];
  int32_t n;
...
*/
int32_t ESP_Resp_GetVersion (uint8_t *buf, uint32_t len) {
  uint8_t crlf[] = {'\r', '\n'};
  ...
  while (val < cnt) {
    ...
    val += BufRead(&buf[val], (uint32_t)n, &ESP.resp);
  }

  /* Flush any leftovers */
  do {
    n = BufReadByte(&ESP.resp);
  }
  while (n != -1);
...
/**
  ...
  \return 0:OK, -1: error
*/
int32_t ESP_Cmd_Echo (uint32_t enable) {
  char out[8];
  int32_t n;
...
/**
  ...
  \return 0:OK, -1: error
*/
int32_t ESP_Cmd_ConfigUART (uint32_t at_cmode, uint32_t baudrate, uint32_t databits, uint32_t stop_par_flowc) {
  char out[32];
...
/**
  ...
  \return
*/
int32_t ESP_Resp_ConfigUART (uint32_t *baudrate, uint32_t *databits, uint32_t *stop_par_flowc) {
  char    *p;
  uint8_t  buf[32];
  int32_t  val;
  uint32_t a;     /* Argument counter */
...
/**
  ...
  \return 0: ok, -1: error
*/
int32_t ESP_Cmd_TxPower (uint32_t tx_power) {
  char out[32];
  int32_t n;
...
*/
int32_t ESP_Cmd_SysMessages (uint32_t n) {
  char out[32];
  int32_t k;
...
/**
  ...
  \return 0: OK, -1: error (invalid mode, etc)
*/
int32_t ESP_Resp_CurrentMode (uint32_t *mode) {
  uint8_t buf[32];
  int32_t val;
  char *p;
...
/**
  ...
  \return 0: ok, -1: error
*/
int32_t ESP_Cmd_ConnectAP (uint32_t at_cmode, const char *ssid, const char *pwd, const char *bssid) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return - 1: connection timeout
          ...
          - 4: connection failed
*/
int32_t ESP_Resp_ConnectAP (ESP_CWJAP *ap) {
  char    *p;
  uint8_t  buf[32+1];
  int32_t  val;
  uint32_t a;     /* Argument counter */
...
/**
  ...
  \return 0:ok, -1: error
*/
int32_t ESP_Cmd_DisconnectAP (void) {
  char out[32];
  int32_t n;
...
/**
  Configure ESP access point (SoftAP must be active)
  
  Format: AT+CWSAP_CUR=<ssid>,<pwd>,<chl>,<ecn>[,<max conn>][,<ssid hidden>]
*/
int32_t ESP_Cmd_ConfigureAP (uint32_t at_cmode, ESP_CWSAP *cfg) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return
*/
int32_t ESP_Resp_ConfigureAP (ESP_CWSAP *cfg) {
  char    *p;
  uint8_t  buf[32+1];
  int32_t  val;
  uint32_t a;     /* Argument counter */
...
*/
int32_t ESP_Resp_ListAP (ESP_CWLAP *ap) {
  char    *p;
  uint8_t  buf[32+1];
  int32_t  val;
  uint32_t a;     /* Argument counter */
...
*/
int32_t ESP_Cmd_StationMAC (uint32_t at_cmode, const uint8_t mac[]) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return execution status
          -1 : error
           0 : MAC retrieved
*/
int32_t ESP_Resp_StationMAC (uint8_t mac[]) {
  uint8_t buf[32];
  int32_t val;
  char *p;
...
/**
  ...
  \return 
*/
int32_t ESP_Cmd_AccessPointMAC (uint32_t at_cmode, uint8_t mac[]) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return execution status
*/
int32_t ESP_Resp_AccessPointMAC (uint8_t mac[]) {
  uint8_t buf[32];
  int32_t val;
  char *p;
...
/**
  Set/Query current IP address of the ESP station
  ...
  Response: ESP_Resp_StationIP
*/
int32_t ESP_Cmd_StationIP (uint32_t at_cmode, uint8_t ip[], uint8_t gw[], uint8_t mask[]) {
  char out[70];
  int32_t n;
...
*/
int32_t ESP_Resp_StationIP (uint8_t addr[]) {
...
/**
  Set/Query current IP address of the ESP access point
  ...
  \return 
*/
int32_t ESP_Cmd_AccessPointIP (uint32_t at_cmode, uint8_t ip[], uint8_t gw[], uint8_t mask[]) {
  char out[70];
  int32_t n;
...
int32_t ESP_Resp_AccessPointIP (uint8_t addr[]) {
  return ESP_Resp_StationIP (addr);
}
...
*/
int32_t ESP_Cmd_DNS (uint32_t at_cmode, uint32_t enable, uint8_t dns0[], uint8_t dns1[]) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return execution status
          ...
          - 1: address list contains more data
*/
int32_t ESP_Resp_DNS (uint8_t addr[]) {
  uint8_t buf[32];
  int32_t val;
  char *p;
...
*/
int32_t ESP_Cmd_DHCP (uint32_t at_cmode, uint32_t mode, uint32_t enable) {
  char out[64];
  int32_t n;
...
*/
int32_t ESP_Resp_DHCP (uint32_t *mode) {
  uint8_t buf[32];
  int32_t val;
  char *p;
...
/**
  Set/Query DHCP IP address lease time and range for ESP access point

  AT command is enabled when ESP8266 runs as SoftAP, and when DHCP is enabled.
...
*/
int32_t ESP_Cmd_RangeDHCP (uint32_t at_cmode, uint32_t en_tlease, uint8_t ip_start[], uint8_t ip_end[]) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return execution status
          ...
          - 0: OK, response retrieved, no more data
*/
int32_t ESP_Resp_RangeDHCP (uint32_t *t_lease, uint8_t ip_start[], uint8_t ip_end[]) {
  char    *p;
  uint8_t  buf[32+1];
  int32_t  val;
  uint32_t a;     /* Argument counter */
...
*/
int32_t ESP_Resp_ListIP (uint8_t ip[], uint8_t mac[]) {
  uint8_t  buf[32]; /* Argument buffer */
  int32_t  val;     /* Control value */
  uint32_t a;       /* Argument counter */
  char *p;
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
int32_t ESP_Cmd_GetStatus (uint32_t at_cmode) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return execution status
          ...
          - 0: OK, response retrieved, no more data
*/
int32_t ESP_Resp_GetStatus (ESP_LINK_CONN *conn) {
  char    *p;
  uint8_t  buf[32];
  int32_t  val;
  uint32_t a;     /* Argument counter */
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
int32_t ESP_Cmd_DnsFunction (uint32_t at_cmode, const char *domain) {
  char out[280];
  int32_t n;
...
/**
  ...
  \return execution status
          ...
          - 0: OK, response retrieved, no more data
*/
int32_t ESP_Resp_DnsFunction (uint8_t ip[]) {
  uint8_t buf[32];
  int32_t val;
  char *p;
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
int32_t ESP_Cmd_ConnOpenTCP (uint32_t at_cmode, uint32_t link_id, const uint8_t ip[], uint16_t port, uint16_t keep_alive) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
int32_t ESP_Cmd_ConnOpenUDP (uint32_t at_cmode, uint32_t link_id, const uint8_t r_ip[], uint16_t r_port, uint16_t l_port, uint32_t mode) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
int32_t ESP_Cmd_ConnectionClose (uint32_t at_cmode, uint32_t link_id) {
  char out[32];
  int32_t n;
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
int32_t ESP_Cmd_Ping (uint32_t at_cmode, const uint8_t ip[], const char *domain) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return execution status
          ...
          - 1: list contains more data
*/
int32_t ESP_Resp_Ping (uint32_t *time) {
  uint8_t buf[32];
  int32_t val;
  char *p;
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
int32_t ESP_Cmd_SendData (uint32_t at_cmode, uint32_t link_id, uint32_t length, const uint8_t remote_ip[], uint16_t remote_port) {
  char out[64];
  int32_t n;
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
int32_t ESP_Cmd_ConnectionMux (uint32_t at_cmode, uint32_t mode) {
  char out[32];
  int32_t n;
...
/**
  ...
  \return execution status
          ...
          - 0: OK, response retrieved, no more data
*/
int32_t ESP_Resp_ConnectionMux (uint32_t *mode) {
  uint8_t buf[32];
  int32_t val;
  char *p;
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
int32_t ESP_Cmd_TcpServer (uint32_t at_cmode, uint32_t mode, uint16_t port) {
  char out[32];
  int32_t n;
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
int32_t ESP_Cmd_TcpServerMaxConn (uint32_t at_cmode, uint32_t num) {
  char out[32];
  int32_t n;
...
/**
  ...
  \return execution status
          ...
          - 0: OK, response retrieved, no more data
*/
int32_t ESP_Resp_TcpServerMaxConn (uint32_t *num) {
  uint8_t buf[32];
  int32_t val;
  char *p;
...
/**
  Determine maximum number of bytes to be sent using ESP_Send_Data.

  \return number of bytes
*/
uint32_t ESP_Send_GetFree (void) {
  uint32_t cnt;
...
*/
uint32_t ESP_Send_Data (const uint8_t *buf, uint32_t len) {
  int32_t rval;
  uint32_t n;
