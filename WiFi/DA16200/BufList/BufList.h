...
/**
  Find the Nth occurence of a data byte in the list buffer and return its offset from current position.

  This function does not move list buffer read pointer.
  \return non-zero if data byte was found, -1 otherwise
*/
extern int32_t BufFindByteNth (uint8_t data, uint8_t nth, BUF_LIST *p);
