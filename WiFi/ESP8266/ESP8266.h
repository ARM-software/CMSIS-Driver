...
#define ESP_NOTIFY_EXECUTE               9
#define ESP_NOTIFY_CONNECTED             0  /* ESP station connected to an AP       */
#define ESP_NOTIFY_GOT_IP                1  /* ESP station got IP address           */
#define ESP_NOTIFY_DISCONNECTED          2  /* ESP station disconnected from an AP  */
#define ESP_NOTIFY_CONNECTION_OPEN       3  /* Network connection is established    */
#define ESP_NOTIFY_CONNECTION_CLOSED     4  /* Network connection is closed         */
#define ESP_NOTIFY_STATION_CONNECTED     5  /* Station connects to the ESP AP       */
#define ESP_NOTIFY_STATION_DISCONNECTED  6  /* Station disconnects from the ESP AP  */
#define ESP_NOTIFY_CONNECTION_RX_INIT    7  /* Connection will start receiving data */
#define ESP_NOTIFY_CONNECTION_RX_DATA    8  /* Connection data is ready to read     */
#define ESP_NOTIFY_REQUEST_TO_SEND       10 /* ESP expects data to transmit         */
#define ESP_NOTIFY_RESPONSE_GENERIC      11 /* Received generic command response    */
#define ESP_NOTIFY_TX_DONE               12 /* Serial transmit completed            */
#define ESP_NOTIFY_OUT_OF_MEMORY         13 /* ESP parser is out of memory          */

/**
  ESP notify callback function.

  \param[in]  event   Callback event code (ESP_NOTIFY codes)
  \param[in]  arg     Event argument
*/
extern void ESP_Notify (uint32_t event, void *arg);

/* Generic responses */
#define ESP_RESP_OK                  0  /* "OK"                */
#define ESP_RESP_ERROR               1  /* "ERROR"             */
#define ESP_RESP_FAIL                2  /* "FAIL"              */
#define ESP_RESP_SEND_OK             3  /* "SEND OK"           */
#define ESP_RESP_SEND_FAIL           4  /* "SEND FAIL"         */
#define ESP_RESP_BUSY_P              5  /* "busy p..."         */
#define ESP_RESP_BUSY_S              6  /* "busy s..."         */
#define ESP_RESP_ALREADY_CONNECTED   7  /* "ALREADY CONNECTED" */
#define ESP_RESP_WIFI_CONNECTED      8  /* "WIFI CONNECTED"    */
#define ESP_RESP_WIFI_GOT_IP         9  /* "WIFI GOT IP"       */
#define ESP_RESP_WIFI_DISCONNECT    10  /* "WIFI DISCONNECT"   */
#define ESP_RESP_ECHO               11  /* (echo)              */
#define ESP_RESP_READY              12  /* "ready"             */
#define ESP_RESP_TXREQ            0xF0  /* ">"                 */
#define ESP_RESP_UNKNOWN          0xFF  /* (unknown)           */
...
typedef struct {
  ...
  uint8_t  cmd_cnt;     /* Number of commands sent */
...
/* ESP parser state */
#define ESP_STATE_ANALYZE     0
#define ESP_STATE_WAIT        1
#define ESP_STATE_FLUSH       2
#define ESP_STATE_RESP_DATA   3
#define ESP_STATE_RESP_GMR    4
#define ESP_STATE_RESP_GEN    5
#define ESP_STATE_RECV_DATA   6
#define ESP_STATE_SEND_DATA   7
#define ESP_STATE_RESP_CTRL   8

extern int32_t ESP_Initialize   (void);
extern int32_t ESP_Uninitialize (void);
extern int32_t ESP_SetBaudrate  (uint32_t baudrate);
extern void    ESP_Execute      (void);
...
/**
  ...
  \return
*/
extern int32_t ESP_Cmd_TestAT (void);
...
/**
  ...
  \return
*/
extern int32_t ESP_Cmd_Reset (void);
...
/**
  ...
  \return
*/
extern int32_t ESP_Cmd_GetVersion (void);
...
/**
  ...
  \return
*/
extern int32_t ESP_Resp_GetVersion (uint8_t *buf, uint32_t len);
...
*/
extern int32_t ESP_Cmd_Echo (uint32_t enable);

/**
  Set/Query the current UART configuration

  ESP8266 supports baudrates in range of [110, 115200*40]

  \note ESP first sends OK response (using old UART settings) and then switches UART config.
  ...
  \return
*/
extern int32_t ESP_Cmd_ConfigUART (uint32_t at_cmode, uint32_t baudrate, uint32_t databits, uint32_t stop_par_flowc);
...
/**
  ...
  \return
*/
extern int32_t ESP_Resp_ConfigUART (uint32_t *baudrate, uint32_t *databits, uint32_t *stop_par_flowc);
...
*/
extern int32_t ESP_Cmd_TxPower (uint32_t tx_power);
...
*/
extern int32_t ESP_Cmd_SysMessages (uint32_t n);
...
*/
extern int32_t ESP_Cmd_CurrentMode (uint32_t at_cmode, uint32_t mode);
...
/**
  ...
  \return
*/
extern int32_t ESP_Resp_CurrentMode (uint32_t *mode);
...
/**
  ...
  \return
*/
extern int32_t ESP_Cmd_ConnectAP (uint32_t at_cmode, const char *ssid, const char *pwd, const char *bssid);
...
/**
  ...
  \return - 1: connection timeout
          ...
          - 4: connection failed
*/
extern int32_t ESP_Resp_ConnectAP (ESP_CWJAP *ap);


/**
  Disconnect from current access point (execute only command).
*/
extern int32_t ESP_Cmd_DisconnectAP (void);
...
/**
  ...
  \return
*/
extern int32_t ESP_Cmd_ConfigureAP (uint32_t at_cmode, ESP_CWSAP *cfg);
...
/**
  ...
  \return
*/
extern int32_t ESP_Resp_ConfigureAP (ESP_CWSAP *cfg);
...
/**
  ...
  Command: CWLAP
*/
extern int32_t ESP_Cmd_ListAP (void);
...
*/
extern int32_t ESP_Resp_ListAP (ESP_CWLAP *ap);
...
/**
  ...
  \return 
*/
extern int32_t ESP_Cmd_StationMAC (uint32_t at_cmode, const uint8_t mac[]);
...
/**
  ...
  \return execution status
*/
extern int32_t ESP_Resp_StationMAC (uint8_t mac[]);
...
/**
  ...
  \return 
*/
extern int32_t ESP_Cmd_AccessPointMAC (uint32_t at_cmode, uint8_t mac[]);
...
/**
  ...
  \return execution status
*/
extern int32_t ESP_Resp_AccessPointMAC (uint8_t mac[]);
...
/**
  ...
  \return 
*/
extern int32_t ESP_Cmd_StationIP (uint32_t at_cmode, uint8_t ip[], uint8_t gw[], uint8_t mask[]);
...
*/
extern int32_t ESP_Resp_StationIP (uint8_t addr[]);
...
/**
  ...
  \return 
*/
extern int32_t ESP_Cmd_AccessPointIP (uint32_t at_cmode, uint8_t ip[], uint8_t gw[], uint8_t mask[]);
...
/**
  ...
  \return execution status
          ...
          - 1: address list contains more data
*/
extern int32_t ESP_Resp_AccessPointIP (uint8_t addr[]);
...
*/
extern int32_t ESP_Cmd_DNS (uint32_t at_cmode, uint32_t enable, uint8_t dns0[], uint8_t dns1[]);
...
/**
  ...
  \return execution status
          ...
          - 1: address list contains more data
*/
extern int32_t ESP_Resp_DNS (uint8_t addr[]);
...
*/
extern int32_t ESP_Cmd_DHCP (uint32_t at_cmode, uint32_t mode, uint32_t enable);
...
*/
extern int32_t ESP_Resp_DHCP (uint32_t *mode);
...
*/
extern int32_t ESP_Cmd_RangeDHCP (uint32_t at_cmode, uint32_t en_tlease, uint8_t ip_start[], uint8_t ip_end[]);
...
/**
  ...
  \return execution status
          ...
          - 0: OK, response retrieved, no more data
*/
extern int32_t ESP_Resp_RangeDHCP (uint32_t *t_lease, uint8_t ip_start[], uint8_t ip_end[]);
...
/**
  ...
  Command: CWLIF
*/
extern int32_t ESP_Cmd_ListIP (void);
...
*/
extern int32_t ESP_Resp_ListIP (uint8_t ip[], uint8_t mac[]);
...
/**
  ...
  Response: Generic response is expected (ESP_RESP_TXREQ);
  ...
  \return execution status:
          0: OK, -1 on error
*/
extern int32_t ESP_Cmd_SendData (uint32_t at_cmode, uint32_t link_id, uint32_t length, const uint8_t remote_ip[], uint16_t remote_port);
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
extern int32_t ESP_Cmd_ConnectionMux (uint32_t at_cmode, uint32_t mode);
...
/**
  ...
  \return execution status
          ...
          - 0: OK, response retrieved, no more data
*/
extern int32_t ESP_Resp_ConnectionMux (uint32_t *mode);
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
extern int32_t ESP_Cmd_TcpServer (uint32_t at_cmode, uint32_t mode, uint16_t port);
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
extern int32_t ESP_Cmd_TcpServerMaxConn (uint32_t at_cmode, uint32_t num);
...
/**
  ...
  \return execution status
          ...
          - 0: OK, response retrieved, no more data
*/
extern int32_t ESP_Resp_TcpServerMaxConn (uint32_t *num);
...
*/
extern int32_t ESP_Cmd_RemoteInfo (uint32_t mode);
...
*/
extern int32_t ESP_Resp_IPD (uint32_t *link_id, uint32_t *len, uint8_t *remote_ip, uint16_t *remote_port);
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
extern int32_t ESP_Cmd_GetStatus (uint32_t at_cmode);
...
/**
  ...
  Note that ESP8266 limits domain name string to 64 characters.
  ...
  \return execution status:
          0: OK, -1 on error
*/
extern int32_t ESP_Cmd_DnsFunction (uint32_t at_cmode, const char *domain);
...
/**
  ...
  \return execution status
          ...
          - 0: OK, response retrieved, no more data
*/
extern int32_t ESP_Resp_DnsFunction (uint8_t ip[]);
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
extern int32_t ESP_Cmd_ConnOpenTCP (uint32_t at_cmode, uint32_t link_id, const uint8_t r_ip[], uint16_t r_port, uint16_t keep_alive);
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
extern int32_t ESP_Cmd_ConnOpenUDP (uint32_t at_cmode, uint32_t link_id, const uint8_t r_ip[], uint16_t r_port, uint16_t l_port, uint32_t mode);
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
extern int32_t ESP_Cmd_ConnectionClose (uint32_t at_cmode, uint32_t link_id);
...
/**
  ...
  \return execution status:
          0: OK, -1 on error
*/
extern int32_t ESP_Cmd_Ping (uint32_t at_cmode, const uint8_t ip[], const char *domain);
...
/**
  ...
  \return execution status
          ...
          - 0: OK, response retrieved, no more data
*/
extern int32_t ESP_Resp_Ping (uint32_t *time);
...
/**
  ...
  Generic responses are: "OK", "ERROR", "SEND OK", "SEND FAIL", etc.
  Definition exist for each generic response: ESP_RESP_OK,
                                              ESP_RESP_ERROR
                                              ESP_RESP_x

  \return response code, see ESP_RESP codes
*/
extern int32_t ESP_Resp_Generic (void);
...
*/
extern uint32_t ESP_Send_GetFree (void);

/**
  Send data (reply to data transmit request).
*/
extern uint32_t ESP_Send_Data (const uint8_t *buf, uint32_t len);
...
*/
extern int32_t ESP_Resp_LinkConn (uint32_t *status, ESP_LINK_CONN *conn);
...
*/
extern int32_t ESP_Resp_CtrlConn (uint32_t *conn_id);
...
*/
extern int32_t ESP_Resp_StaMac (uint8_t mac[]);
